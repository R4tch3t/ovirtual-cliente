...
const PorcentajeDepende = () => {
    ...
    query === ''
      ...
      const handleChange = (op:TypeOpciones) => {
        ...
        const nombreCampo='porcentajeDependeID';
    ...
    return (
        ...
        <Combobox.Label className="block text-sm font-medium text-gray-700"><span className="mt-2 text-sm text-red-500">* </span>¿En qué porcentaje su familia depende de sus ingresos mensuales?:  </Combobox.Label>
        <div className="relative mt-1"  >
            ...
            <Combobox.Options  className="absolute z-999 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                {opFiltrado!.map((op) => (
                <Combobox.Option
                    key={op.nombre+"porcentajeDepende"}
                    value={op}
                    className={({ active }) =>
                    classNames(
        ...
        {paso5?.porcentajeDependeID! === undefined &&
